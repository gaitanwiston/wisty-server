...
//
// Uzito uliopewa unategemea "uwezo" wa pattern kama ulivyoelezwa
// kwenye kitabu: Morning/Evening Star (candles 3, "powerful trend
// reversal signal" - iliyothibitishwa) inapewa uzito sawa na
// Engulfing (20). Dragonfly/Gravestone Doji (candle 1, sawa na Pin
// Bar kimahusiano) inapewa uzito wa Pin Bar (15). Tweezers (candles
// 2, ishara ya wastani) inapewa 12.
...
_log("══════════════════════════════");
    ...
    if (entry <= 0 || atr <= 0) {
      return MarketAnalysisResult(
        ...
        canBuy: false,
        canSell: false,
        // ONGEZO (server hii ya 2 inahitaji field hii - tofauti na
        // server 1, ambayo modeli yake haina 'isValidTrade' kabisa).
        // Uwazi: false hapa ni sahihi kimantiki (data batili
        // haiwezekani kuwa "valid trade").
        isValidTrade: false,
...
_log("══════════════════════════════");
    ...
    return MarketAnalysisResult(
      ...
      // ONGEZO (server hii ya 2 inahitaji field hii kwenye modeli yake
      // - tofauti na server 1). Mantiki: 'canBuy || canSell' - sawa
      // kimantiki na jinsi dhana hii ilivyotumika kila mahali pengine
      // kwenye mfumo huu (mf. runBacktest()).
...
PriceActionAnalysis _detectPriceAction(
  ...
  // Dragonfly Doji: "open, high na close ni sawa (au karibu sawa)" +
  // "mkia mrefu chini" (upper ni ndogo sana/haipo, lower ni kubwa).
  final dragonflyDoji =
      doji &&
      upper <= body * 0.5 &&
      lower > (last.high - last.low) * 0.6;

  // Gravestone Doji: "open na close ni sawa" + "mkia mrefu juu" (lower
  // ni ndogo sana/haipo, upper ni kubwa) - toleo la "bearish" la
  // dragonfly.
  ...
  // Morning Star (candles 3, bullish reversal): candle ya kwanza
  // bearish, ya pili ndogo (indecision), ya tatu bullish ikifunga
  // JUU ya katikati (midpoint) ya mwili wa candle ya kwanza.
  ...
  // Tweezers Top/Bottom (candles 2): "high"/"low" zinazolingana kati
  // ya candles mbili mfululizo, na rangi tofauti (bullish->bearish
  // kwa Top, bearish->bullish kwa Bottom).
...
// ONGEZO JIPYA (hoja B - kwa ombi la mtumiaji): "No Trade Zone" -
// kama 'true', upande huo umezuiwa KABISA (bei iko karibu mno na
// resistance/support - hatari ya kukwama).
